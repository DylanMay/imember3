 ...
 *
 */
public class ListAdapter_mycard extends ArrayAdapter<Store> {
	...
	public ListAdapter_mycard(Context context, ListView listView) {
		super(context, 0);
		_resource = R.layout.listview_item_mycard;
	...
	public int getCount() {
		...
		return ImemberApplication.getInstance().myCards.size();
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		ListItemViewCache_mycard viewCache = null;

		if (view != null) {
			viewCache = (ListItemViewCache_mycard) view.getTag();
		} else {
			view = _inflater.inflate(_resource,null);
			viewCache = new ListItemViewCache_mycard(view);
			view.setTag(viewCache);
		}
		
		MyCard card = ImemberApplication.getInstance().myCards.get(position);
							...
							public void imageLoaded(Drawable imageDrawable,String imageUrl) {
								...
								if(iv_card_logo != null){
									if(imageDrawable == null){
										iv_card_logo.setBackgroundResource(R.drawable.default_bg);
									} else {
