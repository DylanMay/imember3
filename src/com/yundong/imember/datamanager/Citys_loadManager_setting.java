...
import com.yundong.imember.activity.ChangeCityActivity;
...
public class Citys_loadManager_setting {
	private Context _context;
	private MyDialog _myDialog;
	private GetServiceDataHandler handler_cityList;
	private ArrayList<City> citys;

	public Citys_loadManager_setting(Context context, ListAdapter_store listAdapter_store) {
		_context = context;
		...
		handler_cityList = new GetServiceDataHandler("城市列表", 
			...
			public void process_success_handler(String response) {
				if(ImemberApplication.getInstance().citys.size() > 0){
					ImemberApplication.getInstance().citys.clear();
				}
				try {
...
//					intent.putExtra("citys", citys);
					intent.setClass(_context, ChangeCityActivity.class);
